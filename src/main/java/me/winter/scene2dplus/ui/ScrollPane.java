...
import me.winter.scene2dplus.utils.ActorGestureListener;
import me.winter.scene2dplus.utils.Cullable;
import me.winter.scene2dplus.utils.Drawable;
import me.winter.scene2dplus.utils.Layout;
...
public class ScrollPane extends WidgetGroup {
	...
	public void layout() {
		...
		if(!scrollbarsOnTop) {
			if(scrollY) {
				...
				if(!scrollX && widgetWidth > widgetArea.width && !disableX)
					scrollX = true;
			}
			if(scrollX) {
	...
	public void scrollTo(float x, float y, float width, float height, boolean centerHorizontal, boolean centerVertical) {
		...
		if(centerVertical) {
			...
			if(amountY > maxY - y - height + widgetArea.height)
				amountY = maxY - y - height + widgetArea.height;
			if(amountY < maxY - y)
				amountY = maxY - y;
		}
		scrollY(MathUtils.clamp(amountY, 0, maxY));
