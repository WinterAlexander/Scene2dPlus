...
import me.winter.scene2dplus.ui.ScrollPane;
...
public class DragAndDrop {
		...
		DragListener listener = new DragListener() {
			...
			public void drag(InputEvent event, float x, float y, int pointer) {
				...
				if(actor != oldDragActor) {
					if(oldDragActor != null && removeDragActor)
						oldDragActor.remove();
					dragActor = actor;
			...
			public void dragStop(InputEvent event, float x, float y, int pointer) {
				...
				if(System.currentTimeMillis() < dragValidTime)
					isValidTarget = false;
				if(dragActor != null && removeDragActor)
					dragActor.remove();
				if(isValidTarget) {
					float stageX = event.getStageX() + touchOffsetX, stageY = event.getStageY() + touchOffsetY;
					target.actor.stageToLocalCoordinates(tmpVector.set(stageX, stageY));
	 ...
	 * @author Nathan Sweet
	 */
	static public abstract class Source {
		final Actor actor;
		 ...
		 */
		public abstract @Null Payload dragStart(InputEvent event, float x, float y, int pointer);
	 ...
	 * @author Nathan Sweet
	 */
	static public abstract class Target {
		final Actor actor;
		 ...
		 */
		public abstract boolean drag(Source source, Payload payload, float x, float y, int pointer);
